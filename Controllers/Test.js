...
initializeGridFS()
  ...
  // Upload endpoint
  router.post('/uploads', upload.fields([{ name: 'video', maxCount: 1 }, { name: 'poster', maxCount: 1 }]), async (req, res) => {
    try {
  ...
  router.get('/video-files', async (req, res) => {
    try {
      ...
      // Filter files based on metadata.type: "content"
      const documents = await collection.find({ "metadata.type": "content","metadata.vidtype": "Video" }).toArray();
      const documentIds = documents.map(doc => ({id:doc._id,name: doc.metadata.name,vidtype: doc.metadata.vidtype}));
      res.json(documentIds);
    } catch (error) {
      console.error('Error:', error);
  ...
  router.get('/movie-files', async (req, res) => {
    try {
      // Access the database from the existing connection
      const db = mongoose.connection.db;
      const collection = db.collection('fs.files');
  ...
  router.get('/cgi-files', async (req, res) => {
    try {
      // Access the database from the existing connection
      const db = mongoose.connection.db;
      const collection = db.collection('fs.files');
...
router.post('/upload',upload.single('video'), async (req, res) => {
    try {
      ...
      const fileData = req.video.buffer;
      const filename = req.video.originalname;
    //   console.log('gfs:', gfs);
